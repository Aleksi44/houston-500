...
export default class Schema extends Model {
  ...
  modelCreate = () => {
    console.log(this.builder.data);
    this.distance = this.scene.core.options.config.width;
    this.run = false;
    ...
    this.layer = this.map.createStaticLayer(
      0,
      this.tileset,
      this.scene.core.options.config.width,
      0,
    );
  ...
  modelUpdate = (time, delta) => {
    if (this.run) {
      this.distance -= 5;
      if (this.layer.x < (this.builder.getWidth() * -1)) {
        if (this.stop) {
          this.run = false;
          this.stop();
        }
      } else {
        this.layer.setPosition(this.distance, 0);
      }
    }
  };
}
