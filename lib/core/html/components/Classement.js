...
export default class Classement extends React.Component {
  constructor(props) {
    ...
    this.state = {
      scores: [],
      ranking: null,
      inputVal: this.props.core.models.user.getBestPlayer().name,
    };
  }
  ...
  async componentDidMount() {
    ...
    const currentScore = user.getBestCurrentScore();
  ...
  handleSaveScore = async () => {
    const { core } = this.props;
    const { user } = core.models;
    let playerBest = user.getBestPlayer();
    if (playerBest.name) {
      const scores = await core.api.score(
        playerBest.name,
        playerBest.getCurrentScore(),
      );
      this.setState({
        scores,
        ranking: null,
  ...
  setUserName = (username) => {
    const { core } = this.props;
    const { user } = core.models;
    user.getBestPlayer().name = username;
  };
                ...
                <tr>
                    ...
                    <td className="second">
                      <input
                        ...
                        onChange={(e) => {
                          this.props.core.models.user.getBestPlayer().name = e.target.value;
